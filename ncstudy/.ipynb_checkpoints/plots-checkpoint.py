...
def plot_fICurves(cells):
    ...
    for cellid, celln in list(cells.items()):
        if celln.layer == 'L2/3':
...
def plot_ephys_summary(df_ephys, figsize=(12,12)):
    ...
    for ix,col in enumerate(cols):
        ...
        plt.xticks(list(range(3)), fluors)
...
def test_to_string(test_dict, num_comparisons):
    ...
    for pair in list(test_dict['Pairs'].keys()):
        result += '\n%s: %s %.3f'%(pair,
...
def plot_pca(df, figsize=(15, 5)):
    ...
    pairs_ix = list(combinations(list(range(len(df.columns)-1)), r=2))
