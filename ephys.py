...
def steadyStateVoltage(trace,t1,t2):
    '''
    ...
    Arguments:
        ...
        - t1 (float)       : start time in ms to calculates steady state
        - t2 (float)       : end time in ms to calculate steady state
    
    Returns:
        - v_ss (np.float)  : steady state voltage in mV of trace between t1 and t2
    '''
    # convert time points to indices
    start_ix = int(t1*sampling_frequency)
    end_ix   = int(t2*sampling_frequency)
...
def resting_MP(trace):
    ...
    return steadyStateVoltage(trace,0,200)
...
def estimatefISlope(f,I,p0=[100,10]):
    '''
    ...
    popt,pcov = curve_fit(fICurve,I,f,p0=p0)
    return popt
...
def spike_halfwidth(spikeTrace,Vthresh,spikeAmplitude):
    '''
